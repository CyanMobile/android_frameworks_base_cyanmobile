...
class NetworkManagementService extends INetworkManagementService.Stub {
    ...
    class NetdResponseCode {
        ...
        public static final int BtPANListResult           = 114;
    ...
    public void startBtPanNap() throws IllegalStateException {
        mContext.enforceCallingOrSelfPermission(
                android.Manifest.permission.CHANGE_NETWORK_STATE, "NetworkManagementService");
        try {
    ...
    public String[] listBtPanConnections() throws IllegalStateException {
        mContext.enforceCallingOrSelfPermission(
                android.Manifest.permission.CHANGE_NETWORK_STATE, "NetworkManagementService");
        try {
            return mConnector.doListCommand("pan list", NetdResponseCode.BtPANListResult);
        } catch (NativeDaemonConnectorException e) {
            Slog.e(TAG, "listBtPANConnections IllegalStateException");
            throw new IllegalStateException(
                    "Error communicating to native daemon for starting Bluetooth PAN", e);
        }
    }
	
    public void connectBtPan(String adress, String role) throws IllegalStateException {
        mContext.enforceCallingOrSelfPermission(
                android.Manifest.permission.CHANGE_NETWORK_STATE, "NetworkManagementService");
        try {
            mConnector.doCommand(String.format("pan connect " + adress + " " + role));
        } catch (NativeDaemonConnectorException e) {
            throw new IllegalStateException(
                    "Error communicating to native daemon for starting Bluetooth PAN", e);
        }
    ...
    public void disconnectBtPan(String adress) throws IllegalStateException {
        mContext.enforceCallingOrSelfPermission(
                android.Manifest.permission.CHANGE_NETWORK_STATE, "NetworkManagementService");
        try {
            mConnector.doCommand(String.format("pan disconnect " + adress));
        } catch (NativeDaemonConnectorException e) {
            throw new IllegalStateException(
                    "Error communicating to native daemon for starting Bluetooth PAN", e);
        }
