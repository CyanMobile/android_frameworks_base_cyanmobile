...
class BatteryService extends Binder {
    ...
    private int mFullBatteryWarningVoltage;
    ...
    // true because the first (on boot) should be silent.
    private boolean mSuppressNextSound = true;
    ...
    public BatteryService(Context context) {
        ...
        mFullBatteryWarningVoltage = mContext.getResources().getInteger(
                com.android.internal.R.integer.config_fullBatteryWarningVoltage);
        ...
        if (soundCharger != null) {
            mChargingSoundsId = mChargingSounds.load(soundCharger, 1);
        }

        soundCharger = Settings.System.getString(cr, Settings.System.DISCHARGING_BATTERY_SOUND);
        if (soundCharger != null) {
            mDischargingSoundsId = mChargingSounds.load(soundCharger, 1);
        }
    }
    ...
    private Thread batteryinfo_poll_thread = new Thread() {
        ...
        public void run() {
            ...
            poll_freq = SystemProperties.getInt("persist.sys.battery.polling", 1) * 1000;
            try {
                while (true) {
                    sleep(poll_freq);
                    update();
    ...
    private synchronized final void update() {
        ...
        if (mBatteryStatus != mLastBatteryStatus ||
            ...
            /* The ACTION_BATTERY_FULL broadcast is sent in these situations:
             * - is battery level full and battery level falls to WARNING boundary
             *   (becomes >= mFullBatteryWarningLevel).
             */
            final boolean sendBatteryFull = plugged
                && mBatteryStatus != BatteryManager.BATTERY_STATUS_UNKNOWN
                && mBatteryLevel >= mFullBatteryWarningLevel
                && mBatteryVoltage >= mFullBatteryWarningVoltage
                && (oldPlugged || mLastBatteryLevel == mFullBatteryWarningLevel || mLastBatteryVoltage == mFullBatteryWarningVoltage);
            ...
            if (sendBatteryLow) {
                ...
                mContext.sendBroadcast(statusIntent);
            } else if (mSentLowBatteryBroadcast && mLastBatteryLevel >= mLowBatteryWarningLevel) {
                mSentLowBatteryBroadcast = false;
                statusIntent.setAction(Intent.ACTION_BATTERY_OKAY);
                mContext.sendBroadcast(statusIntent);
            } else if (sendBatteryFull) {
                mSentFullBatteryBroadcast = true;
                statusIntent.setAction(Intent.ACTION_BATTERY_FULL);
                mContext.sendBroadcast(statusIntent);
            }
    ...
    private final void playSounds(boolean locked) {
        // User feedback.

        if (mSuppressNextSound) {
            mSuppressNextSound = false;
            return;
        }
