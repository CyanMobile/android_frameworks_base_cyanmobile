...
public class VolumePanel extends Handler implements OnSeekBarChangeListener, View.OnClickListener
{
    ...
    private static final int MSG_SLIDER_VISIBILITY_CHANGED = 8;
    ...
    private enum StreamResources {
        ...
        StreamResources(int streamType, int iconRes, int iconMuteRes, boolean show) {
    ...
    public VolumePanel(final Context context, AudioService volumeService) {
        ...
        context.getContentResolver().registerContentObserver(
                Settings.System.getUriFor(Settings.System.NOTIFICATIONS_USE_RING_VOLUME), false,
                mSettingsObserver);
        context.getContentResolver().registerContentObserver(
    ...
    private void updateSlider(StreamControl sc) {
        ...
        if (sc.streamType == AudioManager.STREAM_RING && isVibrate()) {
    ...
    protected void onShowVolumeChanged(int streamType, int flags) {
        ...
        if (((flags & AudioManager.FLAG_VIBRATE) != 0) &&
                mAudioService.isStreamAffectedByRingerMode(streamType) && isVibrate()) {
