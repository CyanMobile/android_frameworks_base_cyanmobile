...
class CommandQueue extends IStatusBar.Stub {
    ...
    public void setIMEVisible(boolean visible) {
        synchronized (mList) {
            ...
            mHandler.obtainMessage(MSG_SHOW_IME, visible ? 1 : 0, 0, null).sendToTarget();
        }
    }
    ...
    public void toggleQwikWidgets() {
        synchronized (mList) {
            mHandler.removeMessages(MSG_TOGGLE_QWIK_WIDGETS);
            mHandler.obtainMessage(MSG_TOGGLE_QWIK_WIDGETS, 0, 0, null).sendToTarget();
        }
    }
