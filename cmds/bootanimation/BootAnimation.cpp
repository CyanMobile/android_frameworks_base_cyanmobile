...
#define CUSTOM_BOOTANIMATION_FILE "/data/local/bootanimation.zip"
#define SYSTEM_BOOTANIMATION_FILE "/system/media/bootanimation.zip"
#define USER_BOOTANIMATION_FILE "/data/data/com.cyanogenmod.cmbootanimation/files/bootanimation.zip"
#define USER_SHUTDOWNANIMATION_FILE "/data/data/com.cyanogenmod.cmshutdownanimation/files/shutdownanimation.zip"
...
status_t BootAnimation::readyToRun() {
    ...
    char shutviewString[256];
    char shutdownanimate[PROPERTY_VALUE_MAX];
    property_get("persist.sys.shutdownenable", shutdownanimate, "0");
    bool animateShutdown = atoi(shutdownanimate) != 0;
    ...
    if (animateShutdown) {
       ...
       if (((access(USER_BOOTANIMATION_FILE, R_OK) == 0) && 
        ...
        ((access(CUSTOM_BOOTANIMATION_FILE, R_OK) == 0) && 
         (mZip.open(CUSTOM_BOOTANIMATION_FILE) == NO_ERROR)) ||
       ...
       else if (access(CUSTOM_BOOTANIMATION_FILE, R_OK) == 0)
           fd = fopen(CUSTOM_BOOTANIMATION_FILE, "r");
       ...
       if (fd != NULL) {
          ...
          if (crappyBuffer != NULL) {
              ...
              LOGD("Unable to allocate memory to preload the animation");
          }
          fclose(fd);
    }
#endif
...
{
    initTexture(&mAndroid[0], mAssets, "images/squadzone-logo-mask.png");
...
bool BootAnimation::movie()
{
    ...
    char shutdownenable[PROPERTY_VALUE_MAX];
    property_get("persist.sys.nobootsound", bootsound, "1");
    property_get("persist.sys.boot_volume", bootvolume, "0.2");
    property_get("persist.sys.shutdownenable", shutdownenable, "0");

    bool enableSound = atoi(bootsound) != 0;
    float bootVolume = strtof(bootvolume, NULL);
    bool enableShutdown = atoi(shutdownenable) != 1;

    if(enableSound && enableShutdown){
       FILE *mSoundFile = fopen ( "/data/data/com.cyanogenmod.cmbootsound/files/poweron.ogg", "r" );
       if(mSoundFile != NULL) {
          sp<MediaPlayer> mediaplay = new MediaPlayer();
          mediaplay->setDataSource ("/data/data/com.cyanogenmod.cmbootsound/files/poweron.ogg", NULL);
           mediaplay->setVolume (bootVolume, bootVolume);
          mediaplay->prepare();
          mediaplay->start();
       } else {
          sp<MediaPlayer> mediaplay = new MediaPlayer();
          mediaplay->setDataSource ("/system/media/poweron.ogg", NULL);
           mediaplay->setVolume (bootVolume, bootVolume);
          mediaplay->prepare();
          mediaplay->start();
       }
